...
- `PhprLiveF1ValidationGuide` for passive live F1 25 validation checklist generation.
- Devices-page `P-HPR Live F1 Validation` summary and twelve-step checklist.
- Diagnostics report line for telemetry, `DrivingArmed`, paddle listener, shift intent, P-HPR mode, selected-output readiness, coexistence, emergency stop, and physical-validation pending status.
- App-level tests for checklist coverage, ready mock-validation gates, real-mode manual/session-scoped status, and private-data exclusion.
- Documentation in `docs/SIMAGIC_P_HPR_LIVE_F1_VALIDATION.md`.

Not implemented or not claimed in Phase 3G:

- No unattended hardware writes.
- No physical P-HPR validation.
- No completed live F1 25 validation run.
- No GT Neo synthetic input generation.
- No safe-gain, physical latency, stop behavior, sustained-vibration, pedal-mapping, road-feel, slip-feel, or lock-feel claim.
- No persisted real direct-control enable/arm/device selection.
