...
- Phase 3C now completes real road-vibration production integration with independent brake/throttle road scaling, safe settings persistence, route-interval suppression, telemetry freshness and cached `DrivingArmed` gates, SimPro/SimHub conflict blocking, and mock/fake-real tests.
- Phase 3D now completes real wheel-slip and wheel-lock production integration with safe per-effect target/strength/frequency/duration settings, route-interval suppression, telemetry freshness and cached `DrivingArmed` gates, SimPro/SimHub conflict blocking, priority above road and below gear pulse, and mock/fake-real tests.
...
- `PHprRoadVibrationRouter` for production road-vibration routing through the shared P-HPR output boundary.
- Independent brake/throttle real road-vibration settings for enabled state, minimum strength, maximum strength, minimum frequency, maximum frequency, and duration.
- Safe app-settings persistence for real road-vibration preferences without persisting direct-control enablement, arming, or private HID device path.
- Telemetry freshness, haptics running, emergency mute, cached `DrivingArmed`, selected-output readiness, SimPro/SimHub coexistence, safety limiter, and route-interval gates.
- WPF real direct-control settings and diagnostics for road vibration enabled state, per-pedal road settings, and last road route result.
- Mock/fake-real tests for road routing, stale telemetry blocking, `DrivingArmed` blocking, SimPro conflict blocking, command-interval suppression, priority ordering, per-pedal settings persistence, and ASIO independence.
- `docs/SIMAGIC_P_HPR_ROAD_VIBRATION_GUIDE.md`.

Not implemented or not claimed in Phase 3C:

- No real wheel slip or wheel lock production routing.
- No unattended hardware writes.
- No physical P-HPR validation.
- No safe-gain, physical latency, stop behavior, sustained-vibration, pedal-mapping, or road-feel claim.
- No persisted real direct-control enable/arm/device selection.
- No ASIO/BST-1 audio path change.
...
- `PHprSlipLockRouter` for production wheel-slip and wheel-lock routing through the shared P-HPR output boundary.
- Wheel-slip and wheel-lock settings for enabled state, target module, minimum strength, maximum strength, minimum frequency, maximum frequency, and duration.
- Safe app-settings persistence for real slip/lock preferences without persisting direct-control enablement, arming, or private HID device path.
- Telemetry freshness, haptics running, emergency mute, cached `DrivingArmed`, selected-output readiness, SimPro/SimHub coexistence, safety limiter, and route-interval gates.
- Priority ordering where gear pulse remains highest, wheel lock is above wheel slip, and both are above road vibration.
- WPF real direct-control settings and diagnostics for slip/lock enabled state, effect settings, and last slip/lock route result.
- Mock/fake-real tests for slip routing, lock routing, priority, safety gates, SimPro conflict blocking, command-interval suppression, settings persistence, fake-writer command output, and ASIO independence.
- `docs/SIMAGIC_P_HPR_SLIP_LOCK_GUIDE.md`.

Not implemented or not claimed in Phase 3D:

- No unattended hardware writes.
- No physical P-HPR validation.
- No safe-gain, physical latency, stop behavior, sustained-vibration, pedal-mapping, slip-feel, or lock-feel claim.
- No persisted real direct-control enable/arm/device selection.
- No ASIO/BST-1 audio path change.
...
- Devices-page `P-HPR Workflow Summary` for current mode, selected-output status, coexistence, arm state, emergency stop, validation, settings summaries, counters, and warnings.
- Safe `PhprEffectProfileStore` for P-HPR effect preferences beside the existing audio profile.
- P-HPR profile save/load through the Profiles page for shift intent, mock gear, mock pedal effects, real gear pulse, road vibration, wheel slip, and wheel lock preferences.
- Diagnostics report coverage for profile paths, P-HPR workflow mode, mock/real status, coexistence, validation, last write status, settings, errors, and persistence boundaries.
- Expanded app-level user guide covering ASIO/BST-1, P-HPR mock mode, real direct mode, instant gear pulse, road vibration, slip, lock, emergency stop, SimPro/SimHub warnings, recordings/replay, UDP forwarding, profiles, and diagnostics.
- App-level tests for P-HPR effect profile round-trip, clamping, unsupported/corrupt load failure, and exclusion of direct-control arm/device/private state.
- `docs/SIMAGIC_P_HPR_UI_PROFILES_DIAGNOSTICS.md`.

Not implemented or not claimed in Phase 3E:

- No unattended hardware writes.
- No physical P-HPR validation.
- No safe-gain, physical latency, stop behavior, sustained-vibration, pedal-mapping, UI usability, road-feel, slip-feel, or lock-feel claim.
- No persisted real direct-control enable/arm/device selection.
- No raw/private device path or serial data in diagnostics or profiles.
- No ASIO/BST-1 audio path change.
...
- Deterministic synthetic replay tests for P-HPR road vibration, wheel slip, and wheel lock routing from F1 25 v3 packet definitions through `TelemetryReplayService`, `HapticPipelineCoordinator`, `VehicleState`, and safety-limited mock output.
- Replay-driven `DrivingArmedStateService` validation from active-driving telemetry snapshots.
- Stale replay telemetry and emergency mute rejection coverage.
- Profile-style routing coverage for road target and per-effect enable flags.
- Diagnostics text for pipeline input source, replay source file name or in-memory replay status, and replay packet count.
- Documentation in `docs/SIMAGIC_P_HPR_REPLAY_VALIDATION.md`.

Not implemented or not claimed in Phase 3F:

- No unattended hardware writes.
- No physical P-HPR validation.
- No live F1 25 validation.
- No GT Neo paddle input synthesis.
- No safe-gain, physical latency, stop behavior, sustained-vibration, pedal-mapping, road-feel, slip-feel, or lock-feel claim.
- No persisted real direct-control enable/arm/device selection.
- No raw/private capture, device path, or serial data in replay diagnostics.
- No ASIO/BST-1 audio path change.
...
The extended Phase 2 / Phase 3 master prompt authorizes implementing the gated Stage 2Q real-write code path, Stage 2R validation harness, Phase 3A adapter hardening, Phase 3B instant gear-pulse integration, Phase 3C road-vibration integration, Phase 3D wheel-slip/wheel-lock integration, Phase 3E UI/profile/diagnostics workflow, and Phase 3F replay validation. It does not authorize unattended hardware vibration, automated real writes, automatic startup pulses, persisted arming, or claims of physical validation. Through Phase 3F, the real-write code path and validation harness are implemented but remain default-off, manually armed, fake/model tested, and not physically validated.
