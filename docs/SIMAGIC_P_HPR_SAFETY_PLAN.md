...
The approval phrase authorizes only the current session. Direct mode selection does not authorize writes. Arm state does not authorize writes. Authorization is never persisted, logged, exported, or included in diagnostics. It is revoked on interlock trip, emergency stop, shutdown, device selection change, coexistence conflict, HID open failure, and switching away from Direct mode. Non-stop writes recheck authorization and the global interlock immediately before the physical writer is opened/written. Stop and stop-all remain permitted for fail-safe cleanup while unauthorized or latched.

Open-check is real hardware access even though it sends no reports. Dry-run does not authorize writes.
