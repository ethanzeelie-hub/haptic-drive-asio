...
- Stage 26H now routes shell status publication through focused controllers/view-models, keeps effect-tuning saves off the synchronous UI path, and holds `MainWindow.xaml.cs` below the current Stage 26H size guardrail.
...
- Stages 23C through 23K have now extracted Dashboard, Devices, Effects, Routing / Mixer, Telemetry / UDP, Profiles, Testing / Validation, and Advanced / Diagnostics into dedicated view/component seams with selective presenter reuse, while Stage 23K closes the stream with shell-host guardrails and REC-01 closure documentation. `MainWindow.xaml` is now intentionally small, but `MainWindow.xaml.cs` still remains the deliberate composition/runtime shell for page selection, cross-page direct WPF assignment outside extracted presentation slices, runtime snapshot gathering, startup/shutdown sequencing, and execution-heavy lifecycle/safety work.
...
- The repo now has a repeatable local publish script and a GitHub Actions packaging workflow, but it still does not produce an installer, signed binaries, GitHub Releases publication, or automated installer smoke validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- `HapticEffectEngine` is easier to extend internally, but the public effect options/profile/UI/diagnostic surfaces are still explicitly typed to the current BST-1 set.
...
- The recording library can now filter by filename, source metadata, and health text, but it still does not expose deeper workflows such as on-demand packet browsing, packet-type histograms, or persistent sidecar indexes for very large recording sets.
- The repo still does not produce an installer, signed binaries, GitHub Releases publication, or automated installer smoke validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- `HapticEffectEngine` is easier to extend internally, but the public effect options/profile/UI/diagnostic surfaces are still explicitly typed to the current BST-1 set.
- Legacy version-0 app settings, audio profiles, and P-HPR profiles now migrate through a shared planning seam, but the repo still does not have broader cross-file transactional migration, backup retention, rollback orchestration, or automatic repair of multiple persisted documents as one unit.
...
- Release packaging now smoke-checks the publish directory and extracted zip payload, but the repo still does not produce an installer, signed binaries, GitHub Releases publication, or automated install/uninstall validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Active-effect summary text now comes from a generic engine activity list, but the broader effect surface is still explicitly typed in options, profiles, tuning controls, and detailed diagnostics. Adding a brand-new effect still requires coordinated schema and UI work.
- App settings, audio profiles, and P-HPR profiles now recover from a single-file last-known-good backup snapshot, but the repo still does not have broader cross-file rollback orchestration, retained backup history, or coordinated repair of multiple persisted documents as one unit.
- The recording library is now filterable, but it still does not expose deeper workflows such as on-demand packet browsing, packet-type histograms, or persistent sidecar indexes for very large recording sets.
...
- Release packaging now emits and validates a checksum plus JSON manifest alongside the release zip, but the repo still does not produce an installer, signed binaries, GitHub Releases publication, release-note automation, or automated install/uninstall validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Active-effect summary text now comes from a generic engine activity list, but the broader effect surface is still explicitly typed in options, profiles, tuning controls, and detailed diagnostics. Adding a brand-new effect still requires coordinated schema and UI work.
- App settings, audio profiles, and P-HPR profiles now recover from a single-file last-known-good backup snapshot, but the repo still does not have broader cross-file rollback orchestration, retained backup history, or coordinated repair of multiple persisted documents as one unit.
- The recording library is now filterable, but it still does not expose deeper workflows such as on-demand packet browsing, packet-type histograms, or persistent sidecar indexes for very large recording sets.
...
- Local release preparation is now one staged command, but the repo still does not produce an installer, signed binaries, GitHub Releases publication, release-note automation, or automated install/uninstall validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Active-effect summary text now comes from a generic engine activity list, but the broader effect surface is still explicitly typed in options, profiles, tuning controls, and detailed diagnostics. Adding a brand-new effect still requires coordinated schema and UI work.
- App settings, audio profiles, and P-HPR profiles now recover from a single-file last-known-good backup snapshot, but the repo still does not have broader cross-file rollback orchestration, retained backup history, or coordinated repair of multiple persisted documents as one unit.
- The recording library is now filterable, but it still does not expose deeper workflows such as on-demand packet browsing, packet-type histograms, or persistent sidecar indexes for very large recording sets.
...
- The recording library now shows on-demand selected-recording packet histograms for F1 25 captures, but it still does not expose random-access packet browsing, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Active-effect summary text now comes from a generic engine activity list, but the broader effect surface is still explicitly typed in options, profiles, tuning controls, and detailed diagnostics. Adding a brand-new effect still requires coordinated schema and UI work.
- App settings, audio profiles, and P-HPR profiles now recover from a single-file last-known-good backup snapshot, but the repo still does not have broader cross-file rollback orchestration, retained backup history, or coordinated repair of multiple persisted documents as one unit.
...
- The recording library now lets operators copy selected-recording summary/detail/histogram/preview text to the clipboard, but it still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
- App settings, audio profiles, and P-HPR profiles now keep retained backup-history snapshots and can recover after both the primary file and `.lastgood` copy are unusable, but the repo still does not have broader cross-file rollback orchestration or coordinated repair of multiple persisted documents as one unit.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Active-effect summary text now comes from a generic engine activity list, but the broader effect surface is still explicitly typed in options, profiles, tuning controls, and detailed diagnostics. Adding a brand-new effect still requires coordinated schema and UI work.
...
- Selected-recording detail can now be exported as a standalone sanitized local text artifact, but the repo still does not export raw captures, packet-body decode reports, or richer machine-readable packet inspection artifacts.
- Local support bundles can now include sanitized selected-recording detail text when a recording is selected, but they still do not attach raw captures, opt-in recording files, richer runtime incident artifacts, or packet-body decode exports.
- App settings, audio profiles, and P-HPR profiles now keep retained backup-history snapshots and can recover after both the primary file and `.lastgood` copy are unusable, but the repo still does not have broader cross-file rollback orchestration or coordinated repair of multiple persisted documents as one unit.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
...
- Active BST-1 effect summaries and audio-profile effect-control mapping now have stronger app-side typed seams, but the broader effect surface still remains explicitly typed in tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects page and audio-profile control UI are still static and control-by-control. The new profile-control seam reduces flat-contract sprawl, but it does not yet provide data-driven control/card generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Active BST-1 effect summaries plus audio-profile effect values, text, and input mapping now have stronger app-side typed seams, but the broader effect surface still remains explicitly typed in tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects page and audio-profile control UI are still static and control-by-control. The new input seam removes one more flat contract, but it does not yet provide data-driven control/card generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Live Effects-page status mapping now has a dedicated app-side snapshot builder, but the broader effect surface still remains explicitly typed across tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects page and audio-profile control UI are still static and control-by-control. The new status-snapshot seam improves assembly ownership, but it does not yet provide data-driven control/card generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Effects-page, Routing / Mixer, and BST-1 diagnostics section assembly now have dedicated app-side builders, but the broader effect surface still remains explicitly typed across tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects page and audio-profile control UI are still static and control-by-control. The new diagnostics seam reduces more `MainWindow` effect-string assembly, but it does not yet provide data-driven control/card generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Audio-profile hydration now applies profile-name, BST-1 effect, and mixer/safety controls through the extracted page seams instead of one larger `MainWindow` block, but the broader effect surface still remains explicitly typed across tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects, Profiles, and Routing / Mixer control surfaces are still static and control-by-control. The new view-application seam reduces one more shell-level hotspot, but it does not yet provide data-driven control generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Audio-profile hydration and input capture now both run through extracted page seams instead of larger `MainWindow` blocks, but the broader effect surface still remains explicitly typed across tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects, Profiles, and Routing / Mixer control surfaces are still static and control-by-control. The new view-input seam removes one more shell-level hotspot, but it does not yet provide data-driven control generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Audio-profile tuning/save/load/reset feedback now uses a shared workflow feedback planner instead of repeated inline shell branching, but the broader effect surface still remains explicitly typed across tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects, Profiles, and Routing / Mixer control surfaces are still static and control-by-control. The new feedback seam reduces one more shell-level workflow hotspot, but it does not yet provide data-driven control generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
...
- Audio-profile control capture and application now both run through a shared view sync coordinator instead of `MainWindow` manually choreographing those cross-view calls, but the broader effect surface still remains explicitly typed across tuning controls, persisted profile schema, settings hydration, and detailed per-effect diagnostics. Adding a brand-new effect still requires coordinated work across those surfaces.
- The current WPF Effects, Profiles, and Routing / Mixer control surfaces are still static and control-by-control. The new coordinator seam reduces one more shell-level choreography hotspot, but it does not yet provide data-driven control generation, dynamic effect registration, or plugin-style effect metadata.
- The app still ships only one production game adapter: F1 25. `SelectedGameId` exists, but there is still no visible game picker because a second production game does not yet exist.
- Release packaging now carries a generated Markdown handoff summary, but the repo still does not automate signed release publication, GitHub Releases API publication, installer generation, or automated install/uninstall validation.
- Selected-recording inspection now has a structured app-side analysis seam beneath the formatted text output, but the recording workflow still does not expose random-access per-packet browsing, packet-body decode views, persistent sidecar indexes, or richer cross-game recording analysis workflows for very large recording sets.
