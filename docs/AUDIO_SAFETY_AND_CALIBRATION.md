...
- Stage 17 implements native ASIO streaming, but no physical shaker calibration or final latency validation has been performed.
- `NullAudioOutputDevice` remains the default automated-test output.
- The output-owned render callback must not touch UI, disk IO, logging, networking, blocking waits, or async continuations.
- Stale telemetry wall-clock mute renders safety silence until fresh parsed `VehicleState` arrives.
