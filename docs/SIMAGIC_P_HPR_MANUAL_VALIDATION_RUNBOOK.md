...
Stage 2Q does not execute this runbook automatically. A gated real adapter and manual pulse UI exist, but this repository still does not claim completed physical P-HPR safety validation, safe gain, or physical latency without manual-local evidence.
...
- manual-local validation evidence is still required before claiming brake/throttle direct pulses, parameter response, or physical safety behavior as complete,
