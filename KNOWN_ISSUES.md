...
- Stages 23C through 23J have now extracted Dashboard, Devices, Effects, Routing / Mixer, Telemetry / UDP, Profiles, Testing / Validation, and Advanced / Diagnostics into dedicated view/component seams with selective presenter reuse, but `MainWindow.xaml` and `MainWindow.xaml.cs` still remain the deliberate composition/runtime shell for page selection, cross-page direct WPF assignment outside extracted presentation slices, runtime snapshot gathering, startup/shutdown sequencing, and execution-heavy lifecycle/safety work.
