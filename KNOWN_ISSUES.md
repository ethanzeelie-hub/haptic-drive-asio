...
- A local `VID_3670` candidate can be openable while still rejecting the actual HID report write. `IOException:0x80070057` is now classified as report-shape/write-format failure, and output-report-unknown candidates remain blocked from `can pulse`, but the correct real report shape still requires Ethan's supervised hardware validation.
