    ...
    public static TelemetryUdpStatusPresentation Build(TelemetryUdpStatusSnapshot snapshot)
    {
        ...
        var recordingsDetailText = snapshot.RecordingError is not null
            ...
            : snapshot.RecordingActive
                ? snapshot.RecordingLastPacketRelativeTime is null
        ...
        return new TelemetryUdpStatusPresentation(
            ...
            TelemetryUdpPageStatusText: $"{snapshot.ListenerStatusText} on port {snapshot.ListenerPort}; forwarding {snapshot.ForwardedDatagramCount:N0} packet(s); recording {snapshot.RecordingPacketCount:N0}; parsed {snapshot.ParserSuccessCount:N0}; vehicle samples {snapshot.VehicleStateUpdateCount:N0}.");
    }
}
