    ...
    IReadOnlyDictionary<byte, RoadTextureSurfaceProfile> SurfaceProfiles)
{
    ...
    public RoadTextureEvaluatorOptions Normalize()
    {
        return this with
        {
            ...
            Bst1LowSpeedFrequencyHz = Clamp(Bst1LowSpeedFrequencyHz, 20f, 70f),
            Bst1HighSpeedFrequencyHz = Math.Max(Clamp(Bst1HighSpeedFrequencyHz, 30f, 90f), Clamp(Bst1LowSpeedFrequencyHz, 20f, 70f) + 1f),
            Bst1SpeedFrequencyInfluence = Clamp(Bst1SpeedFrequencyInfluence, 0f, 1f),
            Bst1GrainAmount = Clamp(Bst1GrainAmount, 0f, 0.6f),
