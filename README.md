...
- Audio-profile hydration now also applies profile-name, BST-1 effect, and mixer/safety control values through the extracted `ProfilesView`, `EffectsView`, and `RoutingMixerView` seams instead of leaving another large control-assignment block inline in `MainWindow`.
...
- New `.hdrec` recordings now use the resilient v2 format with raw-payload preservation, per-record CRCs, a footer CRC, incomplete-capture metadata, and recovery support when a recording is truncated before a clean footer.
...
The official F1 25 v3 PDF has been extracted into implementation notes under `docs/`. The app now starts a raw UDP listener on port `20778` by default, binds to loopback by default for same-PC safety, counts incoming datagrams, tracks packet rate, shows a no-packet warning in the dashboard, routes received packets through one bounded ingress worker, offers exact raw packet bytes to forwarding/recording without parser reconstruction, validates F1 25 packet headers, parses the Stage 07 core packet bodies, and maps parsed packets into shared last-known `VehicleState` samples.

Forwarding and recording preserve exact packet payload bytes and do not depend on parser or VehicleState success. LAN telemetry is now explicit opt-in and can be paired with an allowed-remote IP list; the UI and diagnostics warn when LAN is enabled without an allowlist. Replay emits recorded packets back as `UdpTelemetryPacket` values without UDP sockets, now stamps replayed packets with fresh receive timestamps, and uses absolute replay deadlines so scheduler overhead does not accumulate into later packets. Stage 15 adds a runtime coordinator that feeds both live and replayed packets through the same parser, VehicleState adapter, existing effects, mixer, safety chain, and `NullAudioOutputDevice` path. Start/Stop Haptics controls that software pipeline, Emergency Mute immediately silences it, profile tuning affects the active effect/mixer/safety configuration, and Null output diagnostics report deterministic consumed buffers, samples, and peaks.
