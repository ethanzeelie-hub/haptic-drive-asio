...
Phase 3J follow-up: direct P-HPR output selection now distinguishes Raw Input metadata from openable Windows HID device-interface candidates. Raw Input metadata-only candidates cannot pass real direct-output gates, and `can pulse` requires a successful no-report open-check plus known HID output-report capability or successful no-command report-shape validation for the selected HID device-interface candidate. Open-check alone is not enough when the output-report length is unavailable.
