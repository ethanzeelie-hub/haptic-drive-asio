...
Phase 3J follow-up: Stage 18 manual ASIO and paddle-bench validation improvements complete.
...
Stage 18 manual validation follow-up adds two deliberately armed test surfaces. `Manual ASIO Bass Shaker Test` routes short 40 Hz or 50 Hz sine pulses through the selected M-Audio / M-Track ASIO driver, selected output channel, Stage 10 mixer, safety chain, limiter, and ASIO output device after ASIO output is selected, armed, and running. The existing synthetic benchmark remains bound to Null output for deterministic automated checks. `Paddle Gear Bench Test` lets mapped GT Neo paddles generate local validation shift intents without recent F1 telemetry, but only after explicit enable and arm. It can route to mock P-HPR output for no-HID validation or to direct P-HPR output only when the strict FeatureReport `0xF1`, 64-byte, open-check, coexistence, approval, emergency-stop, road, slip, and lock gates are green. Normal live-driving shift intent still requires `DrivingArmed` and recent valid telemetry.
...
Phase 3J follow-up: direct P-HPR output selection now distinguishes Raw Input metadata, safe HID registry metadata, and openable Windows HID device-interface candidates. Raw Input and registry metadata-only candidates cannot pass real direct-output gates. `can pulse` requires a successful no-report open-check plus selected HID output-report or feature-report capability, matching selected transport, successful no-command report-shape validation, approval, clear coexistence, and clear emergency stop. The current `VID_3670/PID_0905` path can surface feature report ID `0xF1`, which is shown as the likely F1 EC SET_REPORT-style transport without changing the confirmed F1 EC protocol bytes.
