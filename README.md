...
Stage 2M: Mock gear pulse routing complete. Stage 18 remains the final Phase 1 pre-shaker readiness package.
...
- `src/HapticDrive.Simagic.PHPR.Abstractions`: non-audio P-HPR command, safety limiter, safety-limited mock output, output, mock protocol, and mock-output contracts.
...
- `src/HapticDrive.Actuation`: cached driving-state, Stage 2F shift-intent event evaluation, and Stage 2M mock-only P-HPR gear pulse routing that stay separate from the ASIO audio path.
